...
/**
 * Chart base class for all 2 axis charts. It is responsible for drawing the two axes and the plot content. It contains
 * a list of all content in the plot and implementations of XYChart can add nodes to this list that need to be rendered.
 * <p>
 * <p>
 * It is possible to install Tooltips on data items / symbols. For example the following code snippet installs Tooltip
 * on the 1st data item.
 * <p>
 * <pre><code>
 ...
 */
@Deprecated
public abstract class XYChart<X, Y, T extends XYChart.Data<X, Y>> extends Chart {
     ...
     */
    Series<X, Y, T> begin = null;
    /**
     * This is called when a series is added or removed from the chart
     */
    private final ListChangeListener<Series<X, Y, T>> seriesChanged = new ListChangeListener<Series<X, Y, T>>() {
        @Override
        public void onChanged(Change<? extends Series<X, Y, T>> c) {
            while (c.next()) {
                if (c.getRemoved().size() > 0) {
                    updateLegend();
                }
                for (Series<X, Y, T> series : c.getRemoved()) {
...
//                    seriesDefaultColorIndex --;
                ...
                for (int i = c.getFrom(); i < c.getTo() && !c.wasPermutated(); i++) {
                    final Series<X, Y, T> series = c.getList().get(i);
     ...
     * XYCharts data
     */
    private ObjectProperty<ObservableList<Series<X, Y, T>>> data = new ObjectPropertyBase<ObservableList<Series<X, Y, T>>>() {
        private ObservableList<Series<X, Y, T>> old;

        @Override
        protected void invalidated() {
            final ObservableList<Series<X, Y, T>> current = getValue();
            ...
            if (old != null || current != null) {
                final List<Series<X, Y, T>> removed = (old != null) ? old : Collections.<Series<X, Y, T>>emptyList();
                ...
                if (toIndex > 0 || !removed.isEmpty()) {
                    seriesChanged.onChanged(new NonIterableChange<Series<X, Y, T>>(0, toIndex, current) {
                        @Override
                        public List<Series<X, Y, T>> getRemoved() {
                            return removed;
                        }
                ...
                seriesChanged.onChanged(new NonIterableChange<Series<X, Y, T>>(0, 0, current) {
                    @Override
                    public List<Series<X, Y, T>> getRemoved() {
                        return old;
                    }
    ...
    public final ObservableList<Series<X, Y, T>> getData() {
        return data.getValue();
    }

    public final void setData(ObservableList<Series<X, Y, T>> value) {
        data.setValue(value);
    }

    public final ObjectProperty<ObservableList<Series<X, Y, T>>> dataProperty() {
        return data;
    }
    ...
    /**
     * If this is true and the vertical axis has both positive and negative values then a additional axis line will be
     * drawn at the zero point
     * <p>
     * @defaultValue true
     */
    ...
    /**
     * If this is true and the horizontal axis has both positive and negative values then a additional axis line will be
     * drawn at the zero point
     * <p>
     * @defaultValue true
     */
    ...
    final int getDataSize() {
        final ObservableList<Series<X, Y, T>> data = getData();
    ...
    private void dataItemsChanged(Series<X, Y, T> series, List<Data<X, Y>> removed, int addedFrom, int addedTo, boolean permutation) {
    ...
    /**
     * This method is called when there is an attempt to add series that was set to be removed, and the removal might
     * not have completed.
     * <p>
     * @param series
     */
    void seriesBeingRemovedIsAdded(Series<X, Y, T> series) {
    }

    /**
     * This method is called when there is an attempt to add a Data item that was set to be removed, and the removal
     * might not have completed.
     * <p>
     * @param data
     */
    void dataBeingRemovedIsAdded(Data<X, Y> item, Series<X, Y, T> series) {
    }
     ...
     */
    protected abstract void dataItemAdded(Series<X, Y, T> series, int itemIndex, Data<X, Y> item);
     ...
     */
    protected abstract void dataItemRemoved(Data<X, Y> item, Series<X, Y, T> series);
     ...
     * nodes to getPlotChildren to represent this series. Also you have to handle adding any data items that are already
     * in the series. You may simply call dataItemAdded() for each one or provide some different animation for a whole
     * series being added.
     * <p>
     * @param series The series that has been added
     * @param seriesIndex The index of the new series
     */
    protected abstract void seriesAdded(Series<X, Y, T> series, int seriesIndex);

    /**
     * A series has been removed from the data model but it is still visible on the chart. Its still visible so that you
     * can handle animation for removing it in this method. After you are done animating the data item you must call
     * removeSeriesFromDisplay() to remove the series from the display list.
     * <p>
     * @param series The series that has been removed
     */
    protected abstract void seriesRemoved(Series<X, Y, T> series);
    ...
    protected void updateAxisRange() {
        ...
        if (xData != null || yData != null) {
            for (Series<X, Y, T> series : getData()) {
    ...
    /**
     * Called to update and layout the plot children. This should include all work to updates nodes representing the
     * plot on top of the axis and grid lines etc. The origin is the top left of the plot area, the plot area with can
     * be got by getting the width of the x axis and its height from the height of the y axis.
     */
    ...
    protected final void layoutChartChildren(double top, double left, double width, double height) {
        ...
        if (xa == null || ya == null || xa.getSide() == null || ya.getSide() == null) {
            return;
        }
...
//
     ...
     */
    protected final void removeSeriesFromDisplay(Series<X, Y, T> series) {
        if (series != null) {
            series.setToRemove = false;
        }
        if (begin == series) {
     ...
     */
    protected final Iterator<Series<X, Y, T>> getDisplayedSeriesIterator() {
        return new Iterator<Series<X, Y, T>>() {
            private boolean start = true;
            private Series<X, Y, T> current = begin;
    ...
    /**
     * The current displayed data value plotted on the X axis. This may be the same as xValue or different. It is used
     * by XYChart to animate the xValue from the old value to the new value. This is what you should plot in any custom
     * XYChart implementations. Some XYChart chart implementations such as LineChart also use this to animate when data
     * is added or removed.
     */
    ...
    /**
     * The current displayed data value plotted on the Y axis. This may be the same as yValue or different. It is used
     * by XYChart to animate the yValue from the old value to the new value. This is what you should plot in any custom
     * XYChart implementations. Some XYChart chart implementations such as LineChart also use this to animate when data
     * is added or removed.
     */
    ...
    /**
     * The current displayed data extra value. This may be the same as extraValue or different. It is used by XYChart to
     * animate the extraValue from the old value to the new value. This is what you should plot in any custom XYChart
     * implementations.
     */
     ...
     * recently deleted that are in the process of being faded out. This creates and returns a iterator over that list.
     * This is what implementations of XYChart should use when plotting data.
     * <p>
     * @param series The series to get displayed data for
     * @return iterator over currently displayed items from this series
     */
    protected final Iterator<Data<X, Y>> getDisplayedDataIterator(final Series<X, Y, T> series) {
    ...
    /**
     * This should be called from dataItemRemoved() when you are finished with any animation for deleting the item from
     * the chart. It will remove the data item from showing up in the Iterator returned by getDisplayedDataIterator().
     * <p>
     * @param series The series to remove
     * @param item The item to remove from series's display list
     */
    protected final void removeDataItemFromDisplay(Series<X, Y, T> series, Data<X, Y> item) {
        series.removeDataItemRef(item);
    }

    private void animate(KeyFrame keyFrame, KeyFrame keyFrame0) {
     ...
     * @since JavaFX 2.0
     */
    public static final class Data<X, Y> extends HashMap {
        // -------------- PUBLIC PROPERTIES ----------------------------------------
         ...
         */
        private Series<X, Y, ? extends Data<X, Y>> series;

        void setSeries(Series<X, Y, ? extends Data<X, Y>> series) {
            this.series = series;
        }
        ...
        private ObjectProperty<X> xValue = new ObjectPropertyBase<X>() {
            ...
            protected void invalidated() {
                ...
                get();
                if (series != null) {
                    XYChart<X, Y, ? extends Data<X, Y>> chart = series.getChart();
                    if (chart != null) {
        ...
        /**
         * The generic data value to be plotted in any way the chart needs. For example used as the radius for
         * BubbleChart.
         */
         ...
         * used by XYChart to animate the xValue from the old value to the new value. This is what you should plot in
         * any custom XYChart implementations. Some XYChart chart implementations such as LineChart also use this to
         * animate when data is added or removed.
         */
         ...
         * used by XYChart to animate the yValue from the old value to the new value. This is what you should plot in
         * any custom XYChart implementations. Some XYChart chart implementations such as LineChart also use this to
         * animate when data is added or removed.
         */
        ...
        /**
         * The current displayed data extra value. This may be the same as extraValue or different. It is used by
         * XYChart to animate the extraValue from the old value to the new value. This is what you should plot in any
         * custom XYChart implementations.
         */
        ...
        /**
         * Next pointer for the next data item. We maintain a linkedlist of the data items so even after the data is
         * deleted from the list, we have a reference to it
         */
     ...
     * @since JavaFX 2.0
     */
    public static final class Series<X, Y, T extends Data<X, Y>> {
        ...
        /*
         * Next pointer for the next series. We maintain a linkedlist of the serieses so even after the series is
         * deleted from the list, we have a reference to it - needed by BarChart e.g.
         */
        Series<X, Y, T> next = null;
        ...
        private final ListChangeListener<Data<X, Y>> dataChangeListener = new ListChangeListener<Data<X, Y>>() {
            ...
            public void onChanged(Change<? extends Data<X, Y>> c) {
                while (c.next()) {
                    ...
                    if (c.wasPermutated()) {
                        Series<X, Y, T> series = Series.this;
                        if (series == null || series.getData() == null) {
                            return;
                        }
                        Data<X, Y> ptr = begin;
                        for (int i = 0; i < series.getData().size(); i++) {
                            T item = series.getData().get(i);
                            if (i == 0) {
                                begin = item;
                                ptr = begin;
                                begin.next = null;
                    ...
                    // inform chart
                    XYChart<X, Y, T> chart = getChart();
                    if (chart != null) {
                        chart.dataItemsChanged(Series.this,
                                               (List<Data<X, Y>>) c.getRemoved(), c.getFrom(), c.getTo(), c
                                               .wasPermutated());
                    }
                }
            }
         ...
         */
        private final ReadOnlyObjectWrapper<XYChart<X, Y, T>> chart = new ReadOnlyObjectWrapper<XYChart<X, Y, T>>(this, "chart");

        public final XYChart<X, Y, T> getChart() {
            return chart.get();
        }

        private void setChart(XYChart<X, Y, T> value) {
            chart.set(value);
        }

        public final ReadOnlyObjectProperty<XYChart<X, Y, T>> chartProperty() {
         ...
         */
        private final ObjectProperty<ObservableList<T>> data = new ObjectPropertyBase<ObservableList<T>>() {
            private ObservableList<T> old;

            @Override
            protected void invalidated() {
                final ObservableList<T> current = getValue();
                // add remove listeners
                if (old != null) {
                ...
                if (old != null || current != null) {
                    final List<T> removed = (old != null) ? old : Collections.<T>emptyList();
                    final int toIndex = (current != null) ? current.size() : 0;
                    // let data listener know all old data have been removed and new data that has been added
                    if (toIndex > 0 || !removed.isEmpty()) {
                        dataChangeListener.onChanged(new NonIterableChange<T>(0, toIndex, current) {
                            @Override
                    ...
                    // let series listener know all old series have been removed
                    dataChangeListener.onChanged(new NonIterableChange<T>(0, 0, current) {
                        @Override
                        public List<T> getRemoved() {
                            return old;
                        }
        ...
        public final void setData(ObservableList<T> value) {
            data.setValue(value);
        }

        public final ObjectProperty<ObservableList<T>> dataProperty() {
            return data;
        }
         ...
         */
        public Series(ObservableList<T> data) {
            setData(data);
         ...
         * @param data ObservableList of XYChart.Data
         */
        public Series(String name, ObservableList<T> data) {
            this(data);
            setName(name);
        }
        ...
        /*
         * The following methods are for manipulating the pointers in the linked list when data is deleted.
         */
