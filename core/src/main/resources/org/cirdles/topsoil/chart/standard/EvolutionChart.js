...
(function () {
    ...
    chart.draw = function (data) {
        ...
        chart.ar08lim = x.domain();
        chart.ar48lim = y.domain();
    ...
    chart.update = function (data) {
        ...
        var r08lim = dot(chart.ar08lim, lambda.U238 / lambda.Th230);
        var r48lim = dot(chart.ar48lim, lambda.U238 / lambda.U234);

        x.domain(chart.ar08lim);
        y.domain(chart.ar48lim);
        ...
        var isochrons = chart.area.clipped.selectAll(".isochron")
                .data(tisochrons);
        ...
        if (any(gt(ar48icntrs, chart.ar48lim[1]))) {
            var iover = find(gt(ar48icntrs, chart.ar48lim[1]));
            ...
            ar48overs.forEach(function (ar48over, iar48over) {
                var root = function (t) {
                    return sub(dot(mxp.UTh_4(t), [1, ar48over * lambda.U238 / lambda.U234, 0]),
                            div(mul(chart.ar48lim[1], lambda.U238), lambda.U234));
                };
        ...
        if (any(lt(ar48icntrs, chart.ar48lim[0]))) {
            var iunder = find(lt(ar48icntrs, chart.ar48lim[0]));
            ...
            ar48unders.forEach(function (ar48under, iar48under) {
                var root = function (t) {
                    return sub(dot(mxp.UTh_4(t), [1, ar48under * lambda.U238 / lambda.U234, 0]),
                            div(mul(chart.ar48lim[0], lambda.U238), lambda.U234));
                };
                var tstart = fzero(root, 50e3);
        ...
        var ar48iContours = chart.area.clipped.selectAll('.ar48iContour')
                .data(ar48icntrs);
        ...
        var zoom = d3.behavior.zoom()
                ...
                .on("zoom", function () {
                    ...
                    chart.ar08lim = [zoom.x().domain()[0], zoom.x().domain()[1]];
                    chart.ar48lim = [zoom.y().domain()[0], zoom.y().domain()[1]];
